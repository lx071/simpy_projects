...
def runCompile(dut_path, top_module_file_name, sim_folder):
    ...
    # 由硬件设计文件得到C++模型以及相关文件
    compile_command_1 = f"verilator -CFLAGS -I../../ -CFLAGS -fPIC -CFLAGS -m64 -CFLAGS -shared -CFLAGS -Wno-attributes -LDFLAGS -fPIC -LDFLAGS -m64 -LDFLAGS -shared -LDFLAGS -Wno-attributes -CFLAGS {pybind_CFLAGS} -CFLAGS -fvisibility=hidden -LDFLAGS -fvisibility=hidden -CFLAGS -DTRACE --Mdir verilator --cc {vfn} --trace --exe {hfn}"
    # 得到相关库文件(.o)以及可执行文件
